...
export class NhomNhaCungCapComponent implements OnInit {
  ...
  onLoadData():void{
    this.subscriptions.add(this.nhomnhacungcapService.findNhomNhaCungCaps().subscribe(
      data => {
  ...
  onRowDelete(id): void{
    ...
    result.then((dialogResult) => {
      if(dialogResult){
        //gọi service xóa
        this.subscriptions.add(this.nhomnhacungcapService.deleteNhomNhaCungCap(id).subscribe(
          data => {
            if(data){
              notify({
                width: 320,
