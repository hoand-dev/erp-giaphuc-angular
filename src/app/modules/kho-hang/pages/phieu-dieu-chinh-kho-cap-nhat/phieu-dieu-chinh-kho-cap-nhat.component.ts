...
@Component({
    selector: 'app-phieu-dieu-chinh-kho-cap-nhat',
    templateUrl: './phieu-dieu-chinh-kho-cap-nhat.component.html',
    styleUrls: ['./phieu-dieu-chinh-kho-cap-nhat.component.css']
})
export class PhieuDieuChinhKhoCapNhatComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.subscriptions.add(
            this.activatedRoute.params.subscribe((params) => {
                let phieudieuchinhkho_id = params.id;
                // lấy thông tin
                if (phieudieuchinhkho_id) {
                    this.subscriptions.add(
                        this.phieudieuchinhkhoService.findPhieuDieuChinhKho(phieudieuchinhkho_id).subscribe(
                            (data) => {
    ...
    onFormFieldChanged(e) {
        ...
        //     this.phieudieuchinhkho.nhacungcap_id = e.value ? e.value.id : null;
        // }
        ...
        if (e.dataField == 'khodieuchinh_id') {
            this.isValidForm = true;
            this.hanghoas.forEach((v, i) => {
    ...
    public onHangHoaChangeRow(col: string, index: number, e: any) {
        switch (col) {
            ...
            /* case 'chietkhau':
                ...
                if (e.value != 0) {
                    this.phieudieuchinhkho.chietkhau = 0;
                }
                break;
            case 'thuevat':
                this.hanghoas[index].thuevat = e.value;
                if (e.value != 0) {
                    this.phieudieuchinhkho.thuevat = 0;
                }
                break; */
        }
    ...
    private onTinhTien() {
        ...
        //this.phieudieuchinhkho.tongtienhang = tongtienhang;
        //this.phieudieuchinhkho.tongthanhtien = tongtienhang - tongtienhang * this.phieudieuchinhkho.chietkhau + (tongtienhang - tongtienhang * this.phieudieuchinhkho.chietkhau) * this.phieudieuchinhkho.thuevat;
    }    
    ...
    public onSubmitForm(e) {
        ...
        this.subscriptions.add(
            this.phieudieuchinhkhoService.updatePhieuDieuChinhKho(phieudieuchinhkho_req).subscribe(
                (data) => {
                    notify(
                        {
                            width: 320,
