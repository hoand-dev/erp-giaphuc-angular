...
export class PhieuNhapKhoThemMoiComponent implements OnInit {
    ...
    openModal(tuphieu: string) {
        ...
        /* hiển thị modal */
        if (tuphieu == 'muahang') {
            this.bsModalRef = this.modalService.show(DanhSachPhieuMuaHangNCCModalComponent, { class: 'modal-xxl modal-dialog-centered', ignoreBackdropClick: true, keyboard: false, initialState });
        } else this.bsModalRef = this.modalService.show(DanhSachPhieuKhachTraHangModalComponent, { class: 'modal-xxl modal-dialog-centered', ignoreBackdropClick: true, keyboard: false, initialState });
    ...
    private onTinhTien() {
        ...
        this.hanghoas.forEach((v, i) => {
            ...
            let chietkhau = this.phieunhapkho.chietkhau != 0 ? this.phieunhapkho.chietkhau : v.chietkhau;
            let thuevat   = this.phieunhapkho.thuevat   != 0 ? this.phieunhapkho.thuevat   : v.thuevat  ;
            let dongiavat = v.dongia - v.dongia * chietkhau + (v.dongia - v.dongia * chietkhau) * thuevat ;

            // làm tròn đơn giá vat và thành tiền
            if(this.phieunhapkho.loaiphieunhapkho == 'nhapkhachtrahang'){
        ...
        this.phieunhapkho.tongthanhtien = tongthanhtien;
    }
