...
@Component({
    selector: 'app-lenh-vay-cap-nhat',
    templateUrl: './lenh-vay-cap-nhat.component.html',
    styleUrls: ['./lenh-vay-cap-nhat.component.css']
})
export class LenhVayCapNhatComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.subscriptions.add(
            this.activatedRoute.params.subscribe((params) => {
                let lenhvay_id = params.id;
                // lấy thông tin
                if (lenhvay_id) {
                    this.subscriptions.add(
                        this.lenhvayService.findLenhVay(lenhvay_id).subscribe(
                            (data) => {
                                this.lenhvay = data;
                            },
                            (error) => {
    ...
    public onSubmitForm(e) {
        ...
        this.subscriptions.add(
            this.lenhvayService.updateLenhVay(lenhvay_req).subscribe(
                (data) => {
                    notify(
                        {
                            width: 320,
